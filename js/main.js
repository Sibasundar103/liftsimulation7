...
const waitingQueue = [];
let floorLiftAssignments = {}; // Keeps track of how many lifts are assigned to each floor
...
function upHandler(elem) {
  ...
  // Initialize the floorLiftAssignments for the called floor if not already set
  if (!floorLiftAssignments[calledFloor]) {
    floorLiftAssignments[calledFloor] = 0; // Initialize if undefined
  }

  // Check if there are already 2 lifts assigned to this floor
  if (floorLiftAssignments[calledFloor] >= 2) {
    console.log('Maximum 2 lifts already assigned to this floor.');
    return; // Exit function to prevent more than 2 lifts being assigned
  }
  ...
  if (availableLifts[closestLift]) {
    ...
    // Increment the count for this floor's assigned lifts
    floorLiftAssignments[calledFloor] += 1;
    ...
    function afterDoorAnim() {
      setTimeout(() => {
        ...
        // Decrease the count for this floor's assigned lifts
        floorLiftAssignments[calledFloor] -= 1;
